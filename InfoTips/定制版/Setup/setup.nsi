...
; 安装程序初始定义常量
!define PRODUCT_PATH ""
!define PRODUCT_NAME "DTG外汇宝"
!define PRODUCT_VERSION "v1.1"
!define PRODUCT_PUBLISHER ""
!define PRODUCT_Install_KEY "Software\DTG外汇宝\"
...
Section -Post
	CreateShortCut "$DESKTOP\DTG外汇宝.lnk" "$INSTDIR\InfoTips.exe"	
	CreateShortCut "$SMPROGRAMS\DTG外汇宝\DTG外汇宝.lnk" "$INSTDIR\InfoTips.exe"
	CreateShortCut "$SMPROGRAMS\DTG外汇宝\卸载DTG外汇宝.lnk" "$INSTDIR\uninst.exe"
...
Function .onInit
	...
	${if} $INSTDIR == ""
		StrCpy $INSTDIR "$PROGRAMFILES\DTG外汇宝\"
	${EndIf}
FunctionEnd
...
Section Uninstall
	...
	RMDir /r $INSTDIR
	;删除快捷方式
	Delete "$DESKTOP\DTG外汇宝.lnk"
	Delete "$SMPROGRAMS\DTG外汇宝\DTG外汇宝.lnk"
	Delete "$SMPROGRAMS\DTG外汇宝\卸载DTG外汇宝.lnk"
	Delete "$SMPROGRAMS\DTG外汇宝\"
