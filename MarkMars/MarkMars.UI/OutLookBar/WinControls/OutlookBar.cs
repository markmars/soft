...
using MarkMars.UI.OutLookBar.Collections;
using MarkMars.UI.OutLookBar.Win32;
using MarkMars.UI.OutLookBar.General;
using MarkMars.UI.OutLookBar.Menus;
...
namespace MarkMars.UI.OutLookBar.WinControls
{
	...
	public class OutlookBarItem : Component
	{
        ...
        public OutlookBarItem(string text, int imageIndex, object tag, Color textColor)
		{
			this.text = text;
			this.imageIndex = imageIndex;
			this.tag = tag;
	...
	[ToolboxBitmap(typeof(MarkMars.UI.OutLookBar.WinControls.OutlookBar), 
    "MarkMars.UI.OutLookBar.WinControls.OutlookBar.bmp")]
	public class OutlookBar : System.Windows.Forms.Control
	{
		...
		public OutlookBar()
		{
			...
			// Load drag cursor
			Assembly myAssembly = Assembly.GetAssembly(Type.GetType("MarkMars.UI.OutLookBar.WinControls.OutlookBar"));
            Stream cursorStream = myAssembly.GetManifestResourceStream("MarkMars.UI.OutLookBar.Resources.DragCursor.cur");
        ...
        // added by chenx 2011-02-12 增加是否可以拖动菜单属性
        public bool IsDrag
        {
						...
						if ( dragging ) 
						{
							...
							else 
							{
								if ( hit == HitTestType.Item )
								{
									...
									if ( itemHighlighted ) 
									{ 
										DrawItem(g, index, itemRect, false, false, Rectangle.Empty, null);
									}
						...
						else 
						{
							if (hit == HitTestType.Item && currentIndex == index)
							{
								...
								if ( Math.Abs(deltaX) >= pixelmov || Math.Abs(deltaY) >= pixelmov )
								{
									...
									Cursor.Current = Cursors.No;
                                    //dragging = true;
									dragging = this.isDrag; // added by chenx 2011-02-12 是否要执行拖动操作
									dragItemIndex = index;
								}
		...
		void DrawHeader(Graphics g, int index, Rectangle rc, Border3DStyle style)
		{
            ...
            //回执背景图
            if (bands[index].BackgroundImage != null)
            {
                g.DrawImage(bands[index].BackgroundImage, new Rectangle(rc.Left, rc.Top, rc.Width+5, rc.Height));
            }
		...
		void DrawItem(Graphics g, int index, Rectangle itemRect, bool hot, bool pressed, Rectangle targetRect, OutlookBarBand drawingBand)
		{
            ...
            Color textColor = band.Items[index].TextColor; //band.TextColor;
			...
			if ( band.IconView == IconView.Large && band.LargeImageList != null ) 
			{
                ...
                // 2010-07-12 added by chenx 增加判断条件 "band.LargeImageList.Images.Count > 0" 用于修正在评分页面点退出系统会出错的问题
                if (band.Items[index].ImageIndex != -1 && band.LargeImageList != null && band.LargeImageList.Images.Count > 0)
                {
                    // Only if we have a valid image index
                    g.SmoothingMode = SmoothingMode.HighQuality;
                    g.DrawImage(band.LargeImageList.Images[band.Items[index].ImageIndex], pt);// 如果"band.LargeImageList.Images.Count == 0 而 band.Items[index].ImageIndex > 0" 那么在评分页面点退出系统时会出错.
